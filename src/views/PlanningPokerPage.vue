        ...
        <div class="center-block">
          <template v-if="!hasSelection">
            <div class="hint-and-waiting">
              <p v-if="tableState === 'waiting_for_players'" class="hint-text">
                Pick a card to join this vote.
              </p>
              <div class="waiting-for-you-slot">
                <div v-if="tableState === 'waiting_for_players'" class="waiting-for-you">
                  Waiting for you
                </div>
              </div>
            </div>
          ...
          <template v-else>
            <div class="center-card-wrapper">
              <div class="center-card" :class="{ flipped: poker.isRevealed }">
      ...
      <div v-if="showRevealModal" class="modal-overlay" @click.self="() => {}">
        <div class="modal">
          ...
          <div class="distribution">
            <h3>Distribution</h3>
            <div class="bar-chart">
              <div v-for="bar in scoreRows" :key="bar.value" class="bar-row">
                <div class="bar-label">
                ...
                <div class="bar-right">
                  <div class="bar-track">
                    <div class="bar-fill" :style="{ width: bar.widthPercent + '%' }">
                      <span v-if="bar.count > 0" class="bar-count">{{ bar.count }}</span>
                    </div>
                  </div>
                  <div v-if="bar.names.length" class="bar-names">
                    <span v-for="name in bar.names" :key="name" class="bar-name">
                      {{ name }}
                    </span>
                  </div>
                </div>
